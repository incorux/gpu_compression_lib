...
__global__ void delta_aafl_decompress_gpu (unsigned char *warp_bit_lenght, unsigned long *warp_position_id, T* compressed_data_block_start, T *compressed_data, T * decompress_data, unsigned long length)
{
    ...
    /* if(data_block_mem * 32 * sizeof(T) * 8 < length) { */
        ...
        if(bit_length > 0)
            ...
            afl_decompress_constant_value_gpu <T, CWARP_SIZE> (bit_length, comp_data_id, data_id, compressed_data, compressed_data_block_start[data_block], decompress_data, length);
    /* } */
}
