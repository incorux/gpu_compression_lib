...
__global__ void avar_decompress_value_gpu (avar_header comp_h, int *compressed_data, int * decompress_data, unsigned long length)
{
    ...
    if (tid < length)
    {
        decompress_data[tid] = avar_decompress_base_value_gpu(comp_h, compressed_data, tid);
    }
}
...
__host__ void run_avar_decompress_value_gpu(avar_header comp_h, int *compressed_data, int *data, unsigned long length)
{
    ...
    avar_decompress_value_gpu <<<block_number, block_size>>> (comp_h, compressed_data, data, length);
}
        ...
        )
{
    ...
    int cblock_id = block * ( WARP_SIZE * comp_h.bit_length)
        + pos_in_warp_lane 
        + ((pos_in_warp_comp_block * comp_h.bit_length) / WORD_SIZE) * WARP_SIZE;

    int bit_pos = pos_in_warp_comp_block * comp_h.bit_length % WORD_SIZE;
    int bit_ret = (WORD_SIZE - bit_pos) >= comp_h.bit_length ? comp_h.bit_length : WORD_SIZE - bit_pos;

    int ret = GETNPBITS(compressed_data[cblock_id], bit_ret, bit_pos);

    if (bit_ret < comp_h.bit_length)
        ret |= GETNBITS(compressed_data[cblock_id+WARP_SIZE], comp_h.bit_length - bit_ret) << bit_ret;
