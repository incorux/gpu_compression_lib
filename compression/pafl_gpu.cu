        ...
        )
{
    ...
    afl_compress_gpu <int, 32> <<<block_number, block_size>>> (comp_h.bit_length, data, compressed_data, length);
        ...
        )
{
    int block_size = WARP_SIZE * 8; // better occupancy
    unsigned long block_number = (length + block_size * WARP_SIZE - 1) / (block_size * WARP_SIZE);
    afl_decompress_gpu <int, 32> <<<block_number, block_size>>> (comp_h.bit_length, compressed_data, data, length);
