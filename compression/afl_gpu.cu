...
template <typename T, char CWARP_SIZE>
__device__ __host__ void afl_decompress_constant_value_gpu (const unsigned int bit_length, unsigned long comp_data_id, unsigned long data_id, T *compressed_data, T value, T *data, unsigned long length)
{
    unsigned long pos_decomp = data_id;
...
#define GFL_SPEC(X, A) \
    ...
    template __device__ __host__ void afl_decompress_constant_value_gpu <X, A> (const unsigned int bit_length, unsigned long comp_data_id, unsigned long data_id, X *compressed_data, X value, X *data, unsigned long length);\
