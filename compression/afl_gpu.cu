...
__host__ void run_afl_compress_gpu(int bit_length, T *data, T *compressed_data, unsigned long length)
{
    ...
    unsigned long block_number = (length + block_size * CWORD_SIZE - 1) / (block_size * CWORD_SIZE);
...
__global__ void afl_compress_gpu (int bit_length, T *data, T *compressed_data, unsigned long length)
{
    ...
    unsigned long data_id = data_block * CWORD_SIZE + warp_lane;
...
__device__  __host__ void afl_compress_base_gpu (int bit_length, unsigned long data_id, unsigned long comp_data_id, T *data, T *compressed_data, unsigned long length)
{
    ...
    for (unsigned int i = 0; i < CWORD_SIZE && pos_data < length; ++i) //TODO: Moze word_size tutaj ?
    {
        v1 = data[pos_data];
        pos_data += CWARP_SIZE;

        if (v1_pos >= CWORD_SIZE - bit_length){
...
__device__ __host__ void afl_decompress_base_gpu (int bit_length, unsigned long comp_data_id, unsigned long data_id, T *compressed_data, T *data, unsigned long length)
{
    ...
    for (unsigned int i = 0; i < CWORD_SIZE && pos_decomp < length; ++i)
    {
        if (v1_pos >= CWORD_SIZE - bit_length){ 
            v1_len = CWORD_SIZE - v1_pos;
...
template __device__ __host__ int afl_decompress_base_value_gpu <int, 64, 32> (int, int*, unsigned long);
template __device__ __host__ long afl_decompress_base_value_gpu <long, 64, 32>(int, long*, unsigned long);
...
template __device__ __host__ void afl_decompress_base_gpu <int, 64, 32> (int, unsigned long comp_data_id, unsigned long data_id, int *compressed_data, int *data, unsigned long length);
template __device__ __host__ void afl_decompress_base_gpu <long, 64, 32> (int, unsigned long comp_data_id, unsigned long data_id, long *compressed_data, long *data, unsigned long length);
...
template __device__  __host__ void afl_compress_base_gpu <int, 64, 32> (int, unsigned long, unsigned long, int *, int *, unsigned long );
template __device__  __host__ void afl_compress_base_gpu <long, 64, 32> (int, unsigned long, unsigned long, long *, long *, unsigned long );
...
template __global__ void afl_decompress_value_gpu < int, 64, 32> ( int bit_length, int *compressed_data , int * decompress_data , unsigned long length);
template __global__ void afl_decompress_value_gpu < long, 64, 32> ( int bit_length, long *compressed_data , long * decompress_data , unsigned long length);
...
template __global__ void afl_decompress_gpu < int, 64, 32> ( int bit_length, int *compressed_data, int * decompress_data, unsigned long length);
template __global__ void afl_decompress_gpu < long, 64, 32> ( int bit_length, long *compressed_data, long * decompress_data, unsigned long length);
...
template __global__ void afl_compress_gpu < long, 64, 32> ( int bit_length, long *data, long *compressed_data, unsigned long length);
template __global__ void afl_compress_gpu < int, 64, 32> ( int bit_length, int *data, int *compressed_data, unsigned long length);
...
template __host__ void run_afl_compress_gpu < int, 64, 32> (int bit_length, int *data, int *compressed_data, unsigned long length);
template __host__ void run_afl_compress_gpu < long, 64, 32> (int bit_length, long *data, long *compressed_data, unsigned long length);
...
template __host__ void run_afl_decompress_gpu < int, 64, 32> (int bit_length, int *data, int *compressed_data, unsigned long length);
template __host__ void run_afl_decompress_gpu < long, 64, 32> (int bit_length, long *data, long *compressed_data, unsigned long length);
...
template __host__ void run_afl_decompress_value_gpu < long, 64, 32> (int bit_length, long *compressed_data , long *data, unsigned long length);
template __host__ void run_afl_decompress_value_gpu < int, 64, 32> (int bit_length, int *compressed_data , int *data, unsigned long length);
